...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not user.check_password(form.password.data):
            # Logging invalid standard login
            app.logger.warning(f"Invalid login attempt from user: {form.username.data}")
            flash('Invalid username or password')
            return redirect(url_for('login'))
        login_user(user, remember=form.remember_me.data)
        
        # Logging successful standard login
...
def authorized():
    ...
    if request.args.get('code'):
        cache = _load_cache()
        # Acquire token from msal app using the authorization code
